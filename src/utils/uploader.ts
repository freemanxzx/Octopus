...
export type UploadConfig = {
  provider: 'base64' | 'picgo' | 'github' | 'qiniu' | 'alioss' | 'txcos';
...
export const uploadImage = async (inputFile: File, config: UploadConfig): Promise<string> => {
  ...
  if (config.provider === 'picgo') {
    ...
    if (parsed.success && parsed.result && parsed.result.length > 0) return parsed.result[0];
    throw new Error(parsed.message || 'PicGo Upload Failed');
  }

  if (config.provider === 'github') {
    if (!config.githubToken || !config.githubRepo) throw new Error('GitHub configuration is incomplete.');
  ...
  if (config.provider === 'qiniu') {
    if (!accessKey || !secretKey || !bucket || !domain) throw new Error('Qiniu configuration is incomplete');
    ...
    const safe64 = (s: string) => s.replace(/\+/g, '-').replace(/\//g, '_');
    const token = `${accessKey}:${safe64(encodedSigned)}:${safe64(encoded)}`;

    return new Promise((resolve, reject) => {
      qiniu.upload(file, dateFilename, token, {}, { region: (region as any) || 'z0' }).subscribe({
        error: (err: any) => reject(err.message),
        complete: (res: any) => resolve(`${domain.replace(/\/$/, '')}/${res.key}`)
      });
    });
  }

  if (config.provider === 'alioss' || config.provider === 'txcos') {
    if (!accessKey || !secretKey || !bucket || !region) throw new Error(`${config.provider.toUpperCase()} configuration is incomplete`);
    ...
    if (!res.ok) throw new Error(`${config.provider.toUpperCase()} Upload Failed: ${res.statusText}`);

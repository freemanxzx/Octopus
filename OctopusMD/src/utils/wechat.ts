import { UploadConfig } from './uploader';

declare global {
  interface Window {
    wechatAPI: {
      fetchAccessToken: (appId: string, appSecret: string) => Promise<{accessToken: string, expiresIn: number}>;
      uploadMaterial: (accessToken: string, filePath: string, mimeType: string, type: string) => Promise<{mediaId: string, url: string}>;
      publishArticle: (accessToken: string, publishOptions: any) => Promise<string>;
    }
  }
}

export async function syncToWechatDraft(htmlContent: string, title: string, config: UploadConfig): Promise<string> {
  ...
  try {
    ...
    let thumbMediaId = config as any; 
